 ...
 * $Id: ZeroBufferFit.cpp 4943 2013-01-04 18:02:32Z janderson $
 * 
...
namespace Isis {
  ...
  ZeroBufferFit::ZeroBufferFit(const HiCalConf &conf) : 
    ...
    _absErr = toDouble(ConfKey(prof, "AbsoluteError", toString(1.0E-4)));
    _relErr = toDouble(ConfKey(prof, "RelativeError", toString(1.0E-4)));

    _sWidth = toInt(ConfKey(prof, "GuessFilterWidth", toString(17)));
    _sIters = toInt(ConfKey(prof, "GuessFilterIterations", toString(1)));

    if ( prof.exists("MaximumIterations") ) {
      setMaxIters(ToInteger(prof("MaximumIterations")));
    }

    _maxLog = toDouble(ConfKey(prof, "MaximumLog", toString(709.0)));
    _badLines = ToInteger(prof("TrimLines"))/ToInteger(prof("Summing"));
    _minLines = toInt(ConfKey(prof,"ZeroBufferFitMinimumLines", toString(100)));
